...
# (docs/SYSTEM_SPEC.md Phase 6b 参照).
...
def _load_manifest() -> list[dict]:
    if not MANIFEST_PATH.exists():
        pytest.skip(
            ...
            "run ``scripts/python/tools/generate_manifest.py`` or checkout a tree with "
            "``scripts/python/tests/fixtures/source-parity-goldens/`` populated."
        )
