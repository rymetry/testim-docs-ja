...
SYSTEM_SPEC.md Phase 6 の gate 条件に明記されている。
...
2. ``test_exclusion_registry_matches_system_spec`` — **marker 無し** (default CI
   で run)。registry と SYSTEM_SPEC 「Self-enforcing cutover gate」table の
   ``(module, attribute)`` tuple 集合が完全一致することを SYSTEM_SPEC 再パースで
...
- 新規 temporary exclusion frozenset を導入する PR は、本 module の
  ``_EXCLUSION_REGISTRY`` に entry を足し、SYSTEM_SPEC 側 table も同 PR で更新
...
class ExclusionEntry:
    ...
    Attributes:
        ...
        reason: なぜ Phase 5 で空にできなかったか (SYSTEM_SPEC 相互参照の手がかり)
    """
...
# docs/SYSTEM_SPEC.md の Phase 6 「Self-enforcing cutover gate」表と
...
def test_all_drift_exclusions_are_empty() -> None:
    ...
    merge 条件とする (docs/SYSTEM_SPEC.md Phase 6 gate 8)。
    """
...
_SYSTEM_SPEC_PATH: Path = Path(__file__).resolve().parents[3] / "docs" / "SYSTEM_SPEC.md"
_SYSTEM_SPEC_GATE_SECTION_MARKER: str = "### Self-enforcing cutover gate"


def _parse_system_spec_exclusion_rows() -> set[tuple[str, str]]:
    """``docs/SYSTEM_SPEC.md`` の「Self-enforcing cutover gate」table を
    parse し、``{(module_dotted, attribute), ...}`` の set を返す。

    table format (SYSTEM_SPEC 側契約):
    ...
    """
    text = _SYSTEM_SPEC_PATH.read_text(encoding="utf-8")
    # section boundary で splice
    marker_idx = text.find(_SYSTEM_SPEC_GATE_SECTION_MARKER)
    if marker_idx < 0:
        pytest.fail(
            f"SYSTEM_SPEC '{_SYSTEM_SPEC_PATH}' に section marker "
            f"'{_SYSTEM_SPEC_GATE_SECTION_MARKER}' が見つからない — doc/registry 同期"
            "契約が崩れている可能性"
        )
    # 次の ``###`` 見出しまでが gate section (または EOF)
    next_section = text.find("\n### ", marker_idx + len(_SYSTEM_SPEC_GATE_SECTION_MARKER))
...
def test_exclusion_registry_matches_system_spec() -> None:
    """registry と SYSTEM_SPEC table の ``(module, attribute)`` tuple が完全一致する。

    cardinality だけでなく **正確な tuple set** を cross-reference する。SYSTEM_SPEC
    ...
    **cutover marker 無し** — Phase 5 coexistence 期間中も default CI で常時 run
    する。registry / SYSTEM_SPEC の同期は cutover より前から維持すべき契約のため、
    ...
    system_spec_rows = _parse_system_spec_exclusion_rows()

    # SYSTEM_SPEC に row がないと registry の存在価値がないので fail
    assert system_spec_rows, (
        f"SYSTEM_SPEC '{_SYSTEM_SPEC_PATH}' の「Self-enforcing cutover gate」table が "
        f"空 or parse できない。table format (``| `tests/foo.py` | `_PY_*_SLUGS` | "
        f"reason |``) を維持すること"
    )

    missing_in_registry = system_spec_rows - registry
    extra_in_registry = registry - system_spec_rows
    if missing_in_registry or extra_in_registry:
        lines = [
            "Registry / SYSTEM_SPEC drift detected — keep both in sync:",
            "",
        ]
        if missing_in_registry:
            lines.append("  SYSTEM_SPEC にあるが registry に未登録:")
            for module, attr in sorted(missing_in_registry):
                lines.append(f"    - {module}.{attr}")
            lines.append("")
        if extra_in_registry:
            lines.append("  registry にあるが SYSTEM_SPEC table に未記載:")
            for module, attr in sorted(extra_in_registry):
                lines.append(f"    - {module}.{attr}")
            lines.append("")
        lines.append(
            "Update both docs/SYSTEM_SPEC.md Phase 6 "
    ...
    **cutover marker 無し** — Phase 5 coexistence 期間中も default CI で常時 run
    ...
    if unregistered:
        pytest.fail(
            ...
            "Add entries to docs/SYSTEM_SPEC.md Phase 6 gate table AND "
