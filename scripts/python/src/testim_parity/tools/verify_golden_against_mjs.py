...
Phase 6b atomic cutover で ``scripts/lib/*.mjs`` / ``scripts/python/tools/
emit_corpus_oracle.mjs`` 等を削除した。committed golden
(``scripts/python/tests/conformance/__oracle__/corpus_golden.jsonl``) は
...
4. ``node scripts/python/tools/emit_corpus_oracle.mjs --out <tmp> --suite all`` 実行
...
retire の際は ``scripts/python/src/testim_parity/tools/verify_golden_against_mjs.py``
と対応する ``tests/test_pipeline_cli_smoke.py::TestVerifyGoldenAgainstMjs`` を
削除。``docs/SYSTEM_SPEC.md`` の gate criteria #14 も削除する。
"""
...
# pre-cutover commit で mjs file 一式と scripts/python/tools/emit_corpus_oracle.mjs が
...
_COMMITTED_GOLDEN = (
    ROOT_DIR / "scripts" / "python" / "tests" / "conformance" / "__oracle__" / "corpus_golden.jsonl"
)
