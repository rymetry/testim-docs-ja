...
# line-level state machine + step/bullet/paragraph counts は ``extract_lines.py``
# に分離済み (code-reviewer MEDIUM 指摘、800 行 soft cap 対応)。本 module では
# re-export するので consumer の import path は変わらない。
from .extract_lines import (  # noqa: E402 — 後続の内部 helper より先に module top で import
    classify_line,
...
# extract_heading_sequence で使う H2-H6 見出し regex (classifyLine は H2-H4 のみ)
