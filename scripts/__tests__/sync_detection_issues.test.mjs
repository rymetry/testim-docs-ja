...
// ---------------------------------------------------------------------------
//
// partial-run guard は managed GitHub issue の同期を拒否する。
// partial run (--slug / --section) の artifacts で managed issue を
// 上書きしないための保護。
//
// Contract:
...
describe('sync-detection-issues partial-run guard', () => {
  ...
  it('proceeds normally when runScope is null (legacy report)', async () => {
    // runScope を持たない legacy report は従来どおり同期対象とみなす。
