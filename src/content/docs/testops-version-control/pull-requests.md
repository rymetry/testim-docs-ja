...
> 🚧 プルリクエストを使用する前に、**Project settings** でプルリクエスト設定を有効化および構成する必要があります。詳細については、[プロジェクト設定](doc:project-settings#pull-request-settings)を参照してください。
...
競合の解決の詳細については、[競合の解決](doc:merging-branches#section-resolving-conflicts)を参照してください。
...
レビュー担当者が PR を送り返した場合、リクエストに応答して PR を再送信できます。レビュー担当者が競合のために PR を送り返した場合は、[競合の解決](doc:merging-branches#section-resolving-conflicts)で説明されているように、競合を確認して解決できます。完了したら、以下の説明に従ってプルリクエストを再送信します。
