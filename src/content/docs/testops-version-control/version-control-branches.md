...
特定のブランチで実行するには、CLI でこのパラメータを使用します
...
現在の開発ブランチに適したテストスイートを実行するには、そのビルドのブランチ名を指定する（--branch）か、**Jenkins**、**CircleCI**、**TravisCI** のいずれかを使用している場合は、次のパラメータを使用すると Testim CLI が自動的に同じブランチのテストを実行します。
