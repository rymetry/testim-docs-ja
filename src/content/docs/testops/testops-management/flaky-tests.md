...
テストは、リトライ後に成功した場合、不安定と見なされます。つまり、最初の試行では失敗し、その後の 1 回以上のリトライで成功した場合です。リトライとその設定方法の詳細については、[コマンドライン CLI](/docs/running-tests/the-command-line-cli#失敗したテストの再試行)をお読みください。
...
この機能は **[テストステータス](/docs/testops/testops-management/test-status)** 機能の一部です。この機能を使用するには、Testim LABS 設定で **Test Status** 機能がオンになっていることを確認してください。\
**不安定なテストを表示するには:**
...
:::note
アプリケーションでバグを識別した場合は、失敗を「Bug in app」としてタグ付けすることもお勧めします。結果タグ付けの詳細については、[失敗タイプによる失敗実行のタグ付け](/docs/results/tag-remote-runs-failures)をご覧ください。
:::
