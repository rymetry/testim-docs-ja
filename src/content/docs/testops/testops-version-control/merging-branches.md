...
:::note
[プルリクエスト](/docs/testops/testops-version-control/pull-requests)機能が有効になっている場合、cherry-pick は PR が承認されてから利用可能になります。
:::
...
:::tip{title="ヒント"}
マージプロセスは、すべてのテストのすべての競合を解決するまで開始されません。ただし、一部のテストで競合を解決し、他の競合を後で解決したい場合は、後で解決したいテストの選択を解除できます。[マージの cherry-pick](#マージの-cherry-pick) セクションを参照してください。選択解除されたテストは、この時点でのマージプロセスの一部にはなりません。
:::
