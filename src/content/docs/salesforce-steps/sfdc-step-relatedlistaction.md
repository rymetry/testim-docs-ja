...
* 新しい関連リストオブジェクトを**作成**します。たとえば、関連する商談を作成します。
...
11. **Expect** セクションの下で、**Add** をクリックして、関連リスト項目に対して実行したい検証を次のように定義します:
    ...
    1. **Column** - ドロップダウンメニューから列の名前を選択します。**Count()** オプションは、関連項目の数をカウントします。たとえば、Column を `Count ()` に、演算子を `==` に、値を `2` に設定すると、関連項目が正確に2つあることを検証します。
    2. **Operator** - 次の演算子のいずれかを選択します:
       1. `==` - 列の値が **Value** フィールドの値と等しい場合。
       2. `!=` - 列の値が **Value** フィールドの値と等しくない場合。
       3. **Contains** - 列の値が **Value** フィールドの値を含む場合。
    3. **Value** - 検証の値。このフィールドではパラメータはサポートされていません。
