...
## Grid上のスケジュール済みテスト実行が確認コードの要求により失敗する

確認コードはデバイスアクティベーションの一部であり、多要素認証とは異なります。Salesforceにログインしてメールで送信されたコードの入力を求められる場合、デバイスアクティベーションが発生しています。

デバイスアクティベーションは、以下のいずれかの条件が存在する場合に発生します:
