---
title: 実行結果一覧
description: 実行結果一覧で実行の結果と統計を表示。実行リスト、統計パネル、グラフ、フィルター、詳細画面、テスト操作。
category: テスト結果
order: 7009
updated: '2025-09-22'
...
実行の全体時間が 90min を超える場合、そのステータスは「RUNNING」から「TIMEOUT」に変わります。ただし、Testim と Grid 間に継続的な接続がないため、タイムアウト期間後もテストはグリッド上で実行されている可能性があります。この場合、すべてのテストが完了すると（実行が完了すると）、実行ステータスは「TIMEOUT」から「FAILED」/「PASSED」に適切に更新されます。
:::
...
<table class="md-table md-table-2cols">
 ...
 <tbody>
  ...
  <tr>
   <td style="text-align: left;">
    &lt;suite name&gt; または<br/>&lt;label name&gt; または<br/>&lt;test plan name&gt;
   </td>
   <td style="text-align: left;">
    <ul><li>CLI を使用して特定のスイートおよび/またはラベルを実行する場合</li><li>CLI を使用して特定のテストプランを実行する場合。CLI または Scheduler を使用して複数のテストプランを実行する場合は、複数の実行エントリが作成され、各テストプランが独自の実行エントリを持ちます。</li></ul>
   </td>
  </tr>
  <tr>
   <td style="text-align: left;">
...
![実行グラフの例](/images/results/execution-runs-screen/1f67add-execution-runs-4.jpg)
...
:::note{title="注意"}
実行 ID は、特定の実行の詳細なデバッグを行う際に役立ちます。
:::
...
4. OR - 選択したラベルの 1 つ以上を含むテストを返します。
5. AND - 選択したすべてのラベルを含むテストのみを返します。
6. **Apply**ボタンをクリックして、実行リストにフィルターを適用します。
