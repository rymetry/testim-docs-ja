...
<table class="md-table md-table-2cols">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td style="text-align: left;">
    テストライブラリから特定のテストをローカルで実行する場合、またはスイートライブラリから特定のスイートをローカルで実行する場合。テストエディターから単一のテストを実行した場合は表示されません。
   </td>
  </tr>
  <tr>
   <td style="text-align: left;">
   ...
   <td style="text-align: left;">
    CLI を使用して特定のスイートおよび/またはラベルを実行する場合、または CLI を使用して特定のテストプランを実行する場合。CLI または Scheduler を使用して複数のテストプランを実行する場合は、複数の実行エントリが作成され、各テストプランが独自の実行エントリを持ちます。
   </td>
  </tr>
  <tr>
   <td style="text-align: left;">
