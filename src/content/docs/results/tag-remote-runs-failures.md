...
リモートで実行された失敗したテストにタグを付け、説明を追加し、以前に報告された課題にリンクしたり、新しい課題を作成したりできます。新しい課題をバグ/課題追跡システムに公開する場合は、最初に Testim と課題追跡システム（a.k.a バグトラッカー）との接続を設定する必要があります。詳細については、[バグトラッカー設定](/docs/integrations/bug-tracker-settings)を参照してください。**テスト失敗タグを追加するには:**
