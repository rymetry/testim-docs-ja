...
外部ソース（CSV、DB など）からのテストデータは、[設定ファイル](/docs/configuration-file-run-hooks)を使用して 1 つまたは複数のテストに渡し、設定ファイルを使用するフラグを追加して CLI でテストを実行できます。
...
:::note{title="設定ファイルの beforeSuite（）フックで提供されるテストデータは、UI で提供されるテストデータを上書きします。"}
