...
このフックは、テスト内の「各ステップの直前・直後」にロジックを挿入したい場合に使用します。
...
フックはテスト構成設定または設定ファイルで作成します。新規または既存の構成で次の方法から設定できます:
...
設定ファイルは、テスト／テストスイートの実行に必要なパラメーターと実行フックを定義する CommonJS 形式のファイルです。バックエンドのセットアップや、単一テスト／すべてのテストの前後で実行したい処理をまとめて定義できます。設定ファイルで設定できるフックタイプは次のとおりです。
...
設定ファイル経由でのフック設定の詳細手順は、以下のガイドを参照してください。設定ファイルの概要については[こちら](/docs/running-tests/configuration-file-run-hooks)を参照してください。フック経由で設定ファイルのパラメーターを追加する方法は[設定ファイルでの定義](/docs/advanced-editing/parameters/configuration-file-parameters#設定ファイルでの定義)を参照してください。
...
:::note
Turbo Mode（高速実行モード）でテストを実行している場合、不要なデータ保存を避けるためフックの表示は制限されます。このモードでは、フックが可視化されるのは失敗した実行のみです。
:::

## Before/After each step フックの可視化

各ステップにフックが設定されている場合、実行後にステップ上へ「Hook」ボタンが表示されます（ステップにカーソルを合わせたときに表示）。
...
:::note
共有グループをフックとして使用している場合、フックはグループ単位ではなく「グループ内部のステップ」に対して表示されます。
:::
...
:::note
フックステップのスクリーンショット表示をサイドバイサイドで確認する場合、基準画像（Baseline）は表示されず、結果側のみが表示されます。
:::

## Before/After test フックの可視化

テスト前後フックはテストごとに 1 回、テストの開始前または終了後に実行されます。実行後、最初のステップ（Setup ステップ）にカーソルを合わせると「Hook」ボタンが表示されます。
...
フックステップは、フックアイコン付きのグレーのボックスとして太い枠線で表示されます。ダブルクリックすると詳細を確認でき、右上の数字はその共有ステップを使用しているテスト数を示します。初期ステップ（Setup step）の右側に矢印が表示されている場合はテスト後フックも存在し、矢印をクリックするとテスト末尾のフック表示に切り替わります。
...
テスト実行後にエラーが発生した場合、そのエラーがフックステップに起因することがあります。フックステップでエラーが発生した場合、関連ステップの左側（各ステップ実行前／テスト実行前フックのエラー）または右側（各ステップ実行後／テスト実行後フックのエラー）に赤いドットが表示されます。
