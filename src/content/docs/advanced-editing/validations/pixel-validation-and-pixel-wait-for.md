...
開始前に、[Applitools Eyes](/docs/integrations/visual-validation) または [LambdaTest Smart UI](/docs/integrations/visual-validation/lambdatest_integration) アプリを Testim と連携してください。詳しくは[ビジュアル検証統合ガイド](/docs/integrations/visual-validation/visual_validation_index)を参照。\
関連情報：

- [Applitools ドキュメント](https://applitools.com/docs/)
- [LambdaTest ドキュメント](https://www.testmuai.com/support/docs/)
...
- **Match level** – ベースラインとテスト間の比較方法を変更したい場合があります。特に動的コンテンツを含むアプリケーションを扱う際に有用です。Testim は次の 4 種類の Applitools Eyes マッチレベル（Exact / Strict / Content / Layout — それぞれ完全一致、厳密一致、コンテンツ一致、レイアウト一致 に相当）をサポートしています。デフォルト値は Strict です。これらのレベルの詳細については [Match Levels](https://applitools.com/docs/common/cmn-eyes-match-levels.html) を参照してください。Testim でマッチレベルを編集するだけでなく、Applitools Eyes で要素、ビューポート、またはページの領域をマークし、異なるマッチレベルを定義することもできます。
...
**Match Level** - 下矢印をクリックし、Applitools Eyes の 4 種類のマッチオプション _Exact_ / _Strict_ / _Content_ / _Layout_ （それぞれ完全一致、厳密一致、コンテンツ一致、レイアウト一致 に相当）から選択します。
