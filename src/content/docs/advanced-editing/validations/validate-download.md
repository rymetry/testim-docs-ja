...
Validate download ステップを追加する一般的な手順は、ダウンロードするファイル形式に関わらず同じです。コードとパラメーターは、ダウンロードするファイルの種類や検証したい属性に応じて変わります。以下の手順（CSV ファイルを例として）の後に、各ファイル形式のサンプルコードとパラメーターを示します。
