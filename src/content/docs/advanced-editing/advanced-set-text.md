...
2. **Text to assign** フィールドの固定テキストを JavaScript 式に置き換えます。\
   例として、一意なユーザー名を設定するには `'user' + Date.now()` と指定します。この式は文字列 `'user'` に、エポック（1970-01-01 00:00:00 UTC）からの経過ミリ秒を連結した値を返します。\
   **Text to assign** フィールドには、文字列（シングル／ダブルクォート）、JavaScript 式、既存の変数、またはそれらをプラス記号で連結した組み合わせを指定できます。
