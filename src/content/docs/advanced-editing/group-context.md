...
:::note
既定では `Context selection mode` が有効になり、繰り返し要素が強調表示されます。通常モードに戻すには、キーボードの `Q` キーを押してください。
:::
...
1. **Properties** パネルの **Context** セクションで **Custom** を選んだ後、キーボードの `Q` キーを押して `Context selection mode` に入ります。
