...
- Browserstack では、サーバーが `--allow-insecure` オプション付きで起動していない旨のエラー（`Execute driver script functionality is not available unless server is started with --allow-insecure`）が返ります。
- Headspin では、コマンド処理中にキーボードを非表示にできない旨のエラー（`An unknown server-side error occurred while processing the command`）が返ります。
