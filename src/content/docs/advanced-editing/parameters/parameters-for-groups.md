...
グループが複数テストで共有されている場合、グループのパラメーターは各テストで参照でき、テストごとに値だけを設定すれば再利用できます。グループへの追加方法は、[ステッププロパティパネルのパラメーター](/docs/advanced-editing/parameters/parameters-in-custom-javascript-steps) と同様です。まず[グループを作成](/docs/editing-tests/groups)し、パラメーターを追加して、グループ内の対象ステップに割り当て、値を設定します。
...
5. 追加のパラメーターも同様に設定します（ステップ 4 を繰り返します）。
