...
Testim では複数の方法で Cookie の設定と取得が行えます。Cookie を「設定」すると属性値がブラウザに書き込まれ、「取得」するとブラウザ内の Cookie 値を取り出して変数に代入できます。\
主な方法は次のとおりです。

- Cookie 専用ステップを使う方法
  - Set Cookie ステップ — テストで利用する Cookie を定義します。HttpOnly や Secure 属性を持つ Cookie の作成にはこのステップが推奨です。
  - Get Cookie ステップ — AUT のブラウザから Cookie を取得し、パラメーターに保存します。以降のステップ（Set Cookie を含む）で参照できます。
- Setup ステップの Test Data プロパティ — 特定のテストでページ読込前に Cookie を読み込ませたい場合に使用します。Cookie のドメイン追加にはこの方法が推奨です。
- カスタム JS ステップ — テスト開始直後でなくても良い場合に、任意の位置で設定／取得を行います。
- 設定ファイル＋実行フック — CLI 実行時に設定ファイル経由で Cookie を設定します。スイート全体で必要な場合に有効です。なお、CLI から Cookie を取得することはできません。
