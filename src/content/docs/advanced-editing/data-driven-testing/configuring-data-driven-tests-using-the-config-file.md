...
後続のデータセットからデータセットパラメーターの 1 つが欠落している場合、例えば 2 番目のデータセットにパスワードが含まれていない場合（つまり `password: walrus` が含まれていない場合）、Testim は最初のデータセットのパラメーターデータ（つまり `belle`）を使用します。**設定ファイルを使用してテストを実行するには:**\
設定ファイルを作成したら、必要に応じてファイルへのパスを指定しつつ、その設定ファイルをパラメーターとしてコマンドライン（CLI）ランナーに渡します。[Testim CLI](/docs/running-tests/the-command-line-cli) からテストを実行すると、テストは順番に複数回実行され、毎回異なるデータセットが使用されます。
