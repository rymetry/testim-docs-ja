...
:fa-arrow-right: **テストライブラリをフィルタリングするには:**
...
> 📘
>
> フィルターをリセットせずに Filter Test ペインを閉じても、フィルターはリセットされず、フィルター条件を満たすテストのみが表示されます。すべてのテストとフォルダーを再度表示するには、Filter Test ペインを再度開き、**Reset filters** をクリックする必要があります。
...
* **除外** - `-` 修飾子を使用して、特定のラベルまたは名前/文字列を除外します。たとえば、「sanity」ラベル内のすべてのテストで失敗しなかったものを取得する場合:
...
*Runs\_screen の \_Suite Runs* タブが *AUT*（テスト対象アプリケーション）ブラウザーと同時に開きます。テストは *AUT\_ブラウザーで実行され、\_Suite Runs* タブで結果を表示できます。
...
:fa-arrow-right: **テスト情報を csv にエクスポートするには:**
