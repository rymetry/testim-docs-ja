...
6. **Dynamic Allocation** オプションを選択した場合、**Rules** で、次の操作を行って条件に一致するデバイスを動的に選択するルールを定義します:
   ...
   2. **Operator** で、デバイスをフィルタリングするために使用される演算子を選択します。たとえば、「greater than」を選択して、OS バージョンがオペランドの値より大きいデバイス（例: 10より大きい）を選択します。場合によっては、利用可能な場合に正規表現オプションを選択することで正規表現フィルターを適用できます。
