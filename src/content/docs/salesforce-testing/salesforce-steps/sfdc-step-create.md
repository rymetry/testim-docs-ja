...
:::note
この時点で別のオブジェクトとレコードタイプを選択したい場合は、**変更**をクリックして再度選択します。接続された Salesforce 環境からのレコードのフィールドのリストが表示されます。(\*) で示されているフィールドは必須です。フィールド名の上にマウスを置くと、Salesforce のフィールドの**データタイプ**と **API 名**が表示されます。
:::
