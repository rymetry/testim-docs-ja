...
3. **Exists** - 指定されたテキストが PDF ドキュメント内に存在するかどうかを検証します。

4. **Not Exists** - 指定されたテキストが PDF ドキュメント内に存在しないことを検証します。

5. **Value** フィールドに、存在または非存在を検証したい文字列を入力します。

次の例では、文字列の非存在をチェックします。

![スクリーンショット](/images/salesforce-steps/sfdc-document-validation/8cd3a51-Picture1.png)

6. 追加の条件を追加するには、**ADD** ボタンを再度クリックして、上記の手順を繰り返します。
