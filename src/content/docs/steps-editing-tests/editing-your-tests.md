...
3. メニューから多数のオプションの中から 1 つを選択して目的のステップを作成し、ドキュメントの他の箇所で説明されているようにステップを編集します。例えば、カスタムアクションを追加する場合は、[カスタムアクションの作成](/docs/custom-validations-and-actions#creating-a-custom-action)を参照してください。
