...
Copado からテストを自動化するには、Copado に [URL Callout ステップ](https://docs.copado.com/articles/#!copado-ci-cd-publication/deployment-step-url-callout) を追加し、単一の Webhook 呼び出しで Testim / TTA for Salesforce の REST API を利用します。
→ **Copado を統合するには:**
...
5. Swagger で [Testim REST API](https://editor.swagger.io/?url=https://raw.githubusercontent.com/testimio/public-openapi/main/api.yaml) に移動し、テスト / テストプラン / テストスイート / テストラベルなどのリモート実行 API 呼び出しの種類を選択して、JSON ペイロードをコピーします。

   ![Swagger 上で Testim REST API エンドポイントを確認する画面](/images/ci-integrations/copado-integration/7c0621b-copado_swagger.png)

6. Copado Release Manager アプリで、Steps セクションに新しいステップを追加してデプロイ (Deployment) を作成します。次の手順に従ってください。
...
10. **URL** に REST API 呼び出しを入力し、URL の末尾にテスト / テストプラン / スイートの ID を追加します。例: `https://api.testim.io/tests/run/234`

11. 以下のヘッダーを追加します。

12. Authorization には、手順 4 の API キーを `Bearer <手順 4 のキー>` の形式で入力します。

13. **Content-Type** に **application/json** を入力します。

14. **Body** に、Swagger でコピーした JSON ペイロード (手順 5) を貼り付けます。

![Copado の URL Callout ステップで Body を設定する画面](/images/ci-integrations/copado-integration/b00424d-copado_steps.png)

15. JSON ペイロード内の `"grid" : "string"` を、Testim / TTA for Salesforce の右上プロフィール内 **Grids** セクションに表示されるグリッド名に置き換えます。
...
テストが完了したら、Steps セクションで View Results をクリックします。このデプロイは、継続的インテグレーション (CI) でも実行できます。

## テストが完了するまで、またはテスト結果に基づいて Copado デプロイを一時停止する

テストまたはテストプランが完了、成功、または失敗するまで、Copado のデプロイを一時停止できます。
→ **Copado を一時停止するには:**
...
2. Testim / TTA for Salesforce のエディターで **Add API action** ステップを作成し、共有ステップにします。
3. **Add API action** ステップに **Resume URL** (手順 1) を入力し、プロパティで **Send via web page** を無効にします。

   ![Add API action ステップに Resume URL を設定する画面](/images/ci-integrations/copado-integration/612d721-copado_pause_step3.png)

4. **Runs > Configuration** リストに移動し、新しい構成を作成して **After test handler** を選択し、共有ステップ (手順 2) を選択します。

   ![Runs 設定で After test handler に共有ステップを設定する画面](/images/ci-integrations/copado-integration/e032878-copado_pause_step4.png)

5. テストまたはテストプランの設定で、新しく作成した構成 (手順 4) を選択します。
