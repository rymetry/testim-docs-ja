...
:::info{title="注意"}
この一括マッピングオプションは、現在 TTM for Jira にテストがないお客様に最適ですが、一括マッピング機能を使用するための要件ではありません。既に TTM for Jira に手動でマッピングしたテストがある場合、この一括マッピング機能は既にマッピングされたテストをスキップします。一括マッピングプロセスに含めたい場合は、[手動でテストのマッピングを解除](#ttm-for-jiraにマッピング済みのテストのマッピング解除)できます。
:::
