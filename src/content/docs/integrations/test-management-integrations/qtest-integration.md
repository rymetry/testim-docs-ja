...
:::info
qTest でテストが変更された場合（新しいバージョン番号）、承認されたバージョンのテストのみが Testim で実行されます。つまり、保存のみをクリックした場合は、以前の承認済みバージョンが実行され、qTest では実行レポートに `unexecuted` ステータスとして反映されます。保存後に承認（Approve）をクリックした場合は、新しい承認済みバージョンが実行されます。
:::
