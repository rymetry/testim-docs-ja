...
テストにサーバーへの認証情報の受け渡しを含むログインプロセスが含まれている場合、モックネットワーク上でテストを実行すると、ログインリクエストがタイムアウトするため、正しく動作しない可能性があります。この問題を解決するには、ログインリクエストを送信する際にパススルー認証を有効にする必要があります。これを行うには、ログインリクエストとパススルー認証プロパティを有効にしたマッピングファイルを追加する必要があります。詳細な手順は**[マッピングファイルのアップロード](/docs/creating-a-mapping-file#マッピングファイルのアップロード)**をご覧ください。
:::
...
1. テストを保存した後（[テストの記録方法](/docs/how-to-record-a-test)を参照）、**プロパティ**アイコンをクリックします。**テストプロパティ**ペインが表示されます:
