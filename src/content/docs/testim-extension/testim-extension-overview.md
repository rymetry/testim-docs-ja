...
Testimはテストの作成と実行にスマートロケーターを使用します。拡張機能を通じて、DOM全体をスキャンし、要素を抽出してその属性をスコアリングします。つまり、拡張機能はDOMへのアクセスを得るための必須コンポーネントであり、Testimビジュアルエディターで新しいテストを作成するために必要です。
...
Testim拡張機能は、アプリケーション外のユーザーのブラウジングを追跡せず、IPアドレスやパスワードなどの個人情報を収集しません。ソースコードを変更することもありません。クライアントとサーバー間のすべての通信は、安全な接続を通じて実行されます。Chromeの拡張機能ガイドラインに準拠しています。
