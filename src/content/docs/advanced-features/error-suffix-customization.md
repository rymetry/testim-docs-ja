...
ステップのプロパティ設定で、失敗時に表示されるエラーメッセージへ任意の文字列（およびパラメーター）をサフィックスとして追加できます。
...
2. **Error suffix** 欄に、失敗時に付加したい文字列やパラメーターを入力します。例: 文字列のみ 'my custom error'、あるいは ‘my custom error’ + `Param1` のように組み合わせ。\
