...
「再利用（Reuse）」はプログラミングにおける基本原則の 1 つです。同じ 20 行のコードを何度もコピーペーストするのではなく、メソッド内に 1 度だけ記述し、必要なときに呼び出します。
...
:fa-arrow-right: **同一テスト内でグループを再利用するには:**
