...
`--grid` `<grid-name>`
Note: Read [here](/docs/grid-management/grid-management) how to configure your grid.
...
Read more [Configuration Library - Web](/docs/test-management/shared-configuration) and [Configuration Library - Mobile](/docs/test-management/configuration-library-mobile).
...
For more info check out: [This Article](/docs/ci-integrations/dedicated-run-tunnel)
...
When running a test with the [Sealights Integration](/docs/other-integrations/sealights-integration) using the CLI, you need to add one of the following IDs from Sealights as options in the Testim CLI execution command:
...
`--sealights-build-session-id`, runs the test with the Sealights Integration using the ID that relates to the specific build that was executed. To obtain the `buildSessionId`, follow the instructions in the [buildSessionID option section](doc:sealights-integration#buildsessionid-option).
...
`--sealights-lab-id`, runs the test with the Sealights Integration using the Lab ID. To obtain the `labId`, follow the instructions in the [labId option section](doc:sealights-integration#labid-option).
