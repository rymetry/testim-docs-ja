...
- Permission for the selected user to connect to the Salesforce environment is:

  - If API Access Control is enabled, “Use Any API Client permission” permission.

  - If API Access Control isn’t enabled, “Approve Uninstalled Connected Apps” permission.
...
- Salesforce account you are executing the test with may not have read permission to view this field.\
    **Possible solution** - to view the field permissions for this account, add a [Permission Validation](/docs/salesforce-steps/sfdc-step-permission-validation) test step and view the permissions.
...
<details>
  ...
  1. Salesforce page load times maybe different between Grid and Local execution. Unexpected additional page load times can cause this timeout error.\
     **Possible solution** - Prior to the failing test step, add the Salesforce step [Wait for Page Load](/docs/salesforce-steps/sfdc-step-waitforpageload). This step  pauses test execution until the Salesforce page has completed loading.
...
<details>
  ...
  **Possible solution -** For concurrent test executions, use a Persona that has been authenticated with username/password. For more information, see [Creating a persona](/docs/salesforce-testing/create-a-persona-and-add-users).
</details>
...
- **Web mode** - this is the regular mode of the recorder, which is indicated by a crossed out cloud icon on the recorder. In this mode every interaction (e.g., click, scroll, add text, etc.) is represented by a separate step.
  ...
  **Possible solution -** If you notice that the recorded steps are not appearing in the editor, you should turn off Salesforce mode by clicking on the cloud icon. For more details, see [Recording Steps](doc:create-a-salesforce-test#recording-steps).
</details>
