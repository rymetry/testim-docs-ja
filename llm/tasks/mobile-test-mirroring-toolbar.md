# 翻訳タスク (mobile-test-mirroring-toolbar)
...
- **DOM/Vision Locate** - DOM モードと Vision Locate モードを切り替えます。DOM モード(デフォルトモード)は、DOM を使用して画面上の要素を見つけます。Vision Locate モードは、ビジュアル分析アルゴリズムを使用して画面をスキャンして要素を見つけます。Vision Locate は、DOM を介して要素が見つからない場合や、WebView 画面の場合に使用されます。詳細については、[Vision Locate](https://help.testim.io/docs/vision-locate) を参照してください。
