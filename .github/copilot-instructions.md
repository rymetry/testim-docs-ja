...
- **フォント**: Noto Sans JP（`public/fonts/noto-sans-jp/` にvendoring。latin + japanese のunicode-range分割variable fontをローカル配信。更新時は `SOURCE.json` と `scripts/__tests__/noto_sans_jp_vendor_assets.test.mjs` を同期する）
