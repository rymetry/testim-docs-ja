...
const issue414Variant = process.env.ISSUE_414_VARIANT ?? 'baseline';

if (!['baseline', 'satteri', 'jsx'].includes(issue414Variant)) {
  throw new Error(
    `ISSUE_414_VARIANT must be baseline, satteri, or jsx (received: ${issue414Variant})`
  );
}
...
async function createSatteriMarkdownProcessor() {
  const [{ satteri }, { satteriCallout }, { satteriHeadingLinks }, { satteriWrapTable }] =
    await Promise.all([
      import('@astrojs/markdown-satteri'),
      import('./src/lib/satteri-callout.ts'),
      import('./src/lib/satteri-heading-links.ts'),
      import('./src/lib/satteri-wrap-table.ts'),
    ]);
  ...
  return satteri({
    ...
    hastPlugins: [satteriHeadingLinks, satteriWrapTable],
  });
}

const markdownProcessor =
  issue414Variant === 'satteri' ? await createSatteriMarkdownProcessor() : unifiedMarkdownProcessor;

if (process.env.NODE_ENV !== 'production') {
  console.log(`[astro] output mode: ${isAuthEnabled ? 'server (SSR)' : 'static'}`);
  console.log(`[astro] Issue #414 variant: ${issue414Variant}`);
}
...
export default defineConfig({
  ...
  compressHTML: issue414Variant === 'jsx' ? 'jsx' : true,
