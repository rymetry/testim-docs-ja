...
翻訳時は原文の構造（見出し・リスト・テーブル・画像・段落）を維持してください。`check:parity` で自動検証されます。

構造契約の詳細（見出しマッピング、`:fa-arrow-right:` 処理、`<details>` 維持、JA のみセクション削除など）は `docs/WRITING_GUIDE.md` の「原文準拠ルール（Source-First 構造契約）」セクションを参照してください。
...
5. **自然な日本語**:
   ...
   - よくある不自然な直訳パターンを避けること:
     ...
     - NG: 「テストエディター」→ OK: 「Test Editor」（画面名は英語維持）
