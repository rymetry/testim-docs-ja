...
**Date:** 2026-04-16
**Status:** Draft（実装着手前。本 plan 自体が deliverable）
...
## 3. Task 一覧（M1: 22 task — うち非ブロッキング 3 件 = T17 / T18 / T19。§3.2 main 表に内包済、別カウントしない）
...
| 列 | 意味 |
|----|------|
| family_tag | 同種指摘の集約タグ（regression / test-coverage / code-quality / abstraction-mixed / error-handling / ドキュメント / 保守性 / test-fragility / test-contract / doc-drift / test-fixture / 凝集度 / security） |
...
| T3 | ARCH-001 | test-coverage | `scripts/__tests__/parity_glossary_mask.test.mjs` | 既存 `describe('maskSegmentText — invariant pattern match')` ブロック | T1 で復元する 5 pattern + PR #286/#287 の他新 pattern それぞれに「example が mask される」「想定外文脈で誤 mask しない」最低 2 ケース追加 | `npm run test -- parity_glossary_mask.test.mjs` green / `grep -c 'inline-js-throw-return\|table-header-pattern\|common-it-loanword\|technical-concept-term\|sfdc-ui-name-with-parens' scripts/__tests__/parity_glossary_mask.test.mjs` ≥ 5 | T1 | 2 |
...
| T5 | ARCH-003 / TEST-NEW-find_untranslated-MISSING-TEST / QA-NEW-find-untranslated-no-tests / AI-002 | test-coverage | `scripts/__tests__/find_untranslated.test.mjs`（新規） | — | 最低 5 ケース: (1) frontmatter 境界 skip / (2) ブロック境界（heading / fence / callout / image） / (3) `--slug=<existing>` filter / (4) baseline 0 件 warning / (5) classifySegment 統合（fully-masked 除外） | `node --test scripts/__tests__/find_untranslated.test.mjs` green / `npm run test` 全体 green / `grep -c '^test(' scripts/__tests__/find_untranslated.test.mjs` ≥ 5 | T11（先行で T11 が `find_untranslated.mjs` を 3 関数に分割していると test 設計が容易） | 3 |
...
| T12 | ARCH-005 / AI-005 / QA-NEW-test-pin-recurrence | test-fragility | `scripts/__tests__/source_parity_align_runtime.test.mjs` | L20 / L200 / L224 / L258 / L271 / L280 の `editing-tests/generating-a-random-value` 単独 pin | 合成 fixture (`scripts/__tests__/fixtures/parity_align_runtime_synthetic.{en,ja}.md`) 新規追加 → pin を合成 fixture に置換。合成化が困難な場合は pin を 2 ページ以上に増やし 1 ページ破綻時の fallback を確保 | `ls scripts/__tests__/fixtures/parity_align_runtime_synthetic.*.md` で 2 ファイル / pin 文字列が fixture path or 2 slug 以上を参照 / `node --test scripts/__tests__/source_parity_align_runtime.test.mjs` green | T2 | 3 |
| T13 | TEST-NEW-align_runtime-PIN-FRAGILITY / TEST-NEW-align_runtime-DOCSTRING-L20 | doc-drift | `scripts/__tests__/source_parity_align_runtime.test.mjs` | docstring L17-23 | swap 履歴（`shared-configuration` → `generating-a-random-value` → 合成 fixture or 複数 pin）と再 pin 判定基準（drift ≤3 で fixture 化等）を docstring に追記 | `grep -A 10 'pin' scripts/__tests__/source_parity_align_runtime.test.mjs \| head -30` に判定基準が文字列で存在 | T12 | 1 |
...
| T20 | B-288-3 | 設計確認 | `src/content/docs/advanced-editing/custom-code-1.md` / `custom-code.md` | 同時編集が source-first 整合かを EN 原文 (`snapshots/en/content/advanced-editing/custom-code-1.html` / `custom-code.html`) と突合し、PR #288 body に根拠 1-2 行追記。整合 NG なら片方の編集を revert | `diff <(turndown snapshots/en/content/advanced-editing/custom-code-1.html) src/content/docs/advanced-editing/custom-code-1.md` で構造一致確認 / PR #288 body に確認結果 1 行 | — | 2 |
...
| T22 | AI-NEW-gather-plan-section-shrinkage | scope-shrinkage | 本 plan 自体の構造 | §1–§12 の必須 7 セクション | 本 plan 起票時点で「依存関係図」「リスクと緩和策」が単行省略されていないか自己点検。本 plan §5（依存関係グラフ）は 22 task 全てに edge を含み、§7（リスク）は R1–R11 で regression / scope-bleed / merge-conflict / baseline-stale / fixture-fragility / scope-creep / upstream-block / review-bypass / numerical-drift の主要 type を網羅 | (a) 本 plan §5 のグラフに T1–T22 の全 node が登場 / (b) §7 のリスク表に R1–R11 が列挙 / (c) §3 header の「22 task」表記が §3.2 表の T# 行数と一致（`grep -cE '^\| T[0-9]+ \|' docs/superpowers/plans/2026-04-16-pr-286-291-stack-remediation.md` = 22）/ (d) §8 footer の「合計 22 task」が §3.2 表の T# 行数と一致 / (e) §3.3 の非ブロッキング 3 件が main 表 T17/T18/T19 と同一エントリであることを脚注で明記 | — | 1 |
...
| G6 | `docs/INVARIANT_TOKENS.md` 登録手順 | T1 の silent drop 再発防止のため、追加 / 削除時の PR body 必須項目（`grep '^## '` 出力 diff 添付）を登録手順 §3-4 として追記 | T1 |
...
G1, G3, G4, G6 ── M2/M3 待ち（M1 完了後に M4 plan で実装）
```
...
| D8 | 5 INVARIANT pattern が PR #291 head に存在 | `grep -c '^## '` = 5 |
...
| **合計** | 22 task（M1 のみ。T17/T18/T19 は §3.3 の非ブロッキングと同一エントリで重複カウントしない） | **35** | 並列で実時間 ~13-16 |
...
| **案 B: 4-5 commit に分割して PR #291 head に積む** | (a) INVARIANT/GLOSSARY (T1-T4)、(b) find_untranslated (T5-T10/T17)、(c) test contract (T11/T14/T15)、(d) test fixture (T12/T13)、(e) PR body / 凝集度 (T16/T18/T19/T20) の 5 commit | 論理単位の review、revert 粒度が適切 | PR #291 が大きくなる |
...
1. **本 plan 自体の review**: 4 reviewers + ai-review で本 plan 文書の網羅性 / 実行可能性 / スコープ規律を再評価（本 run の supervise step 相当）。
...
1. 案 B 採用前提で 5 commit を順に作成:
   - **commit 1（INVARIANT/GLOSSARY）**: T1 / T2 / T3 / T4 / T19（一括 baseline 再生成）
   ...
   - **commit 5（PR body / 凝集度方針 / GLOSSARY 重複検出）**: T16 / T18 / T20 / T21 / T22
2. 各 commit 後に `npm run lint && npm run test && npm run build` を実行
3. 5 commit 全部完了後に `node scripts/generate_parity_baseline.mjs --regenerate` で baseline 最終再生成
4. PR #291 description 更新: §6.2 stack 全体 DoD の D1–D10 チェックリスト埋め
...
| # | 項目 | 解消候補ステップ |
|---|------|------------------|
| Q1 | snapshot 288 vs 284 の数値整合性（filesystem / runtime / ユーザー意図） | M2 で `scripts/check_snapshots.mjs` / `snapshot_diff.mjs` 追跡 |
| Q2 | ベースライン 1731 vs 1297 の stale report（issue #284 body） | issue #284 body の再生成、CI 自動更新が PR #272/#285 merge 後に未反映 |
