...
**期待効果**: `paragraph-count-mismatch` / `step-count-mismatch` が purely normalization 非対称起因で発火していた audit signal の除去。`configure-tricentis-mobile-agent` §19 で frozen baseline 外に残っていた paragraph-count-mismatch (§5.3.1.a carve-out 由来の 1 entry) を含む、同パターン起因の signal が BEFORE/AFTER 差分で減少することを想定。baseline / gate-blocking path は完全不変 (§5.3.N scope lock 遵守)。
