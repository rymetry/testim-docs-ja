...
- 本文中の `https://help.testim.io/docs/{slug}` は、対応する JA ファイルが存在する場合 `/docs/{slug}` に変換する
- HTML の `<a href="...">` 内のリンクも同様に変換する
- `/docs/{slug}` のスラグが実在するファイルを指しているか必ず検証する。英語原文のスラグがリネームされていることがあるため、ファイル名との突き合わせが必要
